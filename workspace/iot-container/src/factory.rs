        ...
        match config.sensor_type.as_str() {
            "BME280" => {
                ...
                Err(IoTError::Configuration(
                    iot_common::ConfigError::UnsupportedFeature("BME280 requires esp32c3 feature")
                ))
            }
            ...
            _ => Err(IoTError::Configuration(
                iot_common::ConfigError::UnsupportedSensorType(config.sensor_type.as_str())
            )),
        }
    }
        ...
        match config.interface_type.as_str() {
            "USB" => {
                #[cfg(feature = "esp32c3")]
                {
                    ...
                    let console = ConsoleAdapter::new(uart_tx, uart_rx, config);
                    Ok(Box::new(console))
                }
                
                #[cfg(not(feature = "esp32c3"))]
                Err(IoTError::Configuration(
                    iot_common::ConfigError::UnsupportedFeature("USB console requires esp32c3 feature")
                ))
            }
            ...
            "UART" => {
                #[cfg(feature = "esp32c3")]
                {
                    let (uart_tx, uart_rx) = platform.get_uart();
                    let console = ConsoleAdapter::new(uart_tx, uart_rx, config);
                    Ok(Box::new(console))
                }
                ...
                #[cfg(not(feature = "esp32c3"))]
                Err(IoTError::Configuration(
                    iot_common::ConfigError::UnsupportedFeature("UART console requires esp32c3 feature")
                ))
            }
            ...
            _ => Err(IoTError::Configuration(
                iot_common::ConfigError::InvalidFormat("Unsupported console interface type")
            )),
        }
    }
}
