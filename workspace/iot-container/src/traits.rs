    ...
    pub fn new(ip_address: &str) -> Result<Self, IoTError> {
        Ok(Self {
            ip_address: EmbeddedString::try_from(ip_address).map_err(|_| {
                IoTError::Configuration(iot_common::ConfigError::InvalidFormat("IP address too long"))
            })?,
            gateway: None,
            subnet_mask: None,
    ...
    pub fn new(device_id: &str, measurements: Measurements) -> Result<Self, IoTError> {
        Ok(Self {
            device_id: EmbeddedString::try_from(device_id).map_err(|_| {
                IoTError::Configuration(iot_common::ConfigError::InvalidFormat("Device ID too long"))
            })?,
            measurements,
            quality_score: if measurements.is_valid() { 1.0 } else { 0.5 },
            sample_count: 1,
        })
    }
    ...
    pub fn new(
        ...
        Ok(Self {
            ...
            status: EmbeddedString::try_from(status).map_err(|_| {
                IoTError::Configuration(iot_common::ConfigError::InvalidFormat("Status string too long"))
            })?,
            uptime_seconds,
            free_heap_bytes,
            wifi_signal_dbm,
