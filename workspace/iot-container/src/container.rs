    ...
    C: ConsoleInterface,
{
    ...
    async fn run_main_loop(&mut self) -> IoTResult<()> {
        ...
        loop {
            ...
            if cycle_start.duration_since(last_status_report) >= status_report_interval {
                if let Err(e) = self.status_report_cycle().await {
    ...
    async fn sensor_reading_cycle(&mut self) -> IoTResult<()> {
        if !self.sensor.is_available().await {
            ...
            return Err(IoTError::Sensor(iot_common::SensorError::NotAvailable("Sensor not responding")));
        }
    ...
    async fn network_operations_cycle(&mut self) -> IoTResult<()> {
        ...
        if let Some(measurements) = SENSOR_DATA_SIGNAL.try_take() {
            if let Err(e) = self.publish_sensor_data(measurements).await {
    ...
    async fn handle_error(&self, error: IoTError) {
        ...
        {
            let mut state = SYSTEM_STATE.lock().await;
            state.last_error_code = match &error {
                IoTError::Sensor(_) => 1,
                IoTError::Network(_) => 2,
                IoTError::Hardware(_) => 3,
                IoTError::Configuration(_) => 4,
                IoTError::System(_) => 5,
            };
        }
